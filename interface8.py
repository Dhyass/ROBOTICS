# -*- coding: utf-8 -*-
"""
Created on Sun Dec 24 10:21:45 2023
...
class RobotController:
    ...
    def create_tool_widgets(self):
        ...
        self.slider_y = ttk.Scale(self.tool_frame, from_=0, to=207.55, variable=self.y_value,
        ...
        self.slider_z = ttk.Scale(self.tool_frame, from_=-65, to=204.55, variable=self.z_value,
    ...
    def create_objects_coordonnes_widgets(self):
        ...
        # Configurer les gestionnaires d'événements pour la touche "Entrée"
        self.entry_x_ob.bind('<Return>', lambda event, entry=self.entry_x_ob: self.handle_enter(event, entry))
        self.entry_y_ob.bind('<Return>', lambda event, entry=self.entry_y_ob: self.handle_enter(event, entry))
        self.entry_z_ob.bind('<Return>', lambda event, entry=self.entry_z_ob: self.handle_enter(event, entry))
        
   
        
    def handle_enter(self, event, entry):
        # Cette méthode sera appelée lorsque la touche "Entrée" est pressée
        x_value = self.entry_x_ob.get()
        y_value = self.entry_y_ob.get()
        z_value = self.entry_z_ob.get()

        # Faites ce que vous voulez avec les valeurs (par exemple, imprimez-les)
    ...
    def robot_home(self):
        ...
        self.calculate_nX()
        self.entry_nx.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ nx
        self.entry_nx.insert(0, str(self.nx.get())) 
        
        self.calculate_ny()
        self.entry_ny.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ ny
        self.entry_ny.insert(0, str(self.ny.get())) 
        
        self.calculate_nz()
        self.entry_nz.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ nz
        self.entry_nz.insert(0, str(self.nz.get()))
        
        self.calculate_sX()
        self.entry_sx.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ sx
        self.entry_sx.insert(0, str(self.sx.get())) 
        
        self.calculate_sy()
        self.entry_sy.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ sy
        self.entry_sy.insert(0, str(self.sy.get())) 
        
        self.calculate_sz()
        self.entry_sz.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ sz
        self.entry_sz.insert(0, str(self.sz.get()))
        
        self.calculate_aX()
        self.entry_ax.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ ax
        self.entry_ax.insert(0, str(self.ax.get())) 
        
        self.calculate_ay()
        self.entry_ay.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ ay
        self.entry_ay.insert(0, str(self.ay.get())) 
        
        self.calculate_az()
        self.entry_az.delete(0, tk.END)  # Supprimez la valeur actuelle dans le champ az
        self.entry_az.insert(0, str(self.az.get()))
        
    def open_second_window(self):
        self.deuxieme_fenetre = tk.Toplevel(self.root)
        self.deuxieme_fenetre.title("Allure x = f(j1)")
        ...
        # Définir la fonction f(t) = 2.5t^2
        t = self.angle_j1.get()
        f_t = self.x_value.get()

        # Tracer la courbe
        ax.plot(t, f_t, label=r'$ x = f(j1)$')
        ax.set_xlabel('j1')
        ax.set_ylabel('X outil')
        ax.legend()

        # Ajouter le graphique à la fenêtre
        canvas = FigureCanvasTkAgg(fig, master=self.deuxieme_fenetre)
